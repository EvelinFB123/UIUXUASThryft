  ...
  <div class="flex flex-wrap items-center justify-between px-6 py-4">
    ...
    <div class="flex items-center space-x-2 mt-2 sm:mt-0">
      ...
      <select onchange="window.location.href='?filter=' + this.value" class="border rounded px-2 py-1 text-sm">
        ...
        <option value="Chair" {{ request('filter') == 'Chair' ? 'selected' : '' }}>Chair</option>
        <option value="Mirror" {{ request('filter') == 'Mirror' ? 'selected' : '' }}>Mirror</option>
        <option value="Wardrobe" {{ request('filter') == 'Wardrobe' ? 'selected' : '' }}>Wardrobe</option>
        <option value="Desk" {{ request('filter') == 'Desk' ? 'selected' : '' }}>Desk</option>
      </select>
    </div>
  </div>
</section>
